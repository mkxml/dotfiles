...
# Keep-alive: update existing `sudo` time stamp until `osconfig.sh` has finished
...
# Note: if you’re in the US, swap the languages and replace `BRL` with `USD`, `Centimeters` with
# `Inches`, `pt_BR` with `en_US` and `true` with `false`.
...
networksetup -setdnsservers Ethernet 1.1.1.1 1.0.0.1 208.67.222.222
networksetup -setdnsservers Wi-Fi 1.1.1.1 1.0.0.1 208.67.222.222
...
# Use the OneDark theme by default in Terminal.app
osascript <<EOD
...
# Auto-play videos when opening with QuickTime
...
# Disable Photos.app automatic opening when plugging an iOS device
